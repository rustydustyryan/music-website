const SONGS: Song[] = (window as any).SONGS = [];
const WEEKS: Week[] = (window as any).WEEKS = [];
...
function addSongs(parentElement: HTMLElement, songs: Song[], includeAudio: boolean) {
...
function addSong(song: Song, songIndex: number, songContainer: HTMLElement, includeAudio: boolean) {
...
function addWeekSongs(week: number, date: string, songs: Song[]) {
  WEEKS.push({
    order: week,
    date: new Date(date),
    songs: songs
